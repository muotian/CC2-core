...
execute on vehicle on origin run tag @s add orange.time_stealing_potion.origin
...
execute at @s positioned ~-7.0 ~-0.5 ~-7.0 as @e[dx=13,dz=13,predicate=orange:is_affected_by_time_stealing,tag=!orange.time_stealing_potion.origin] if function orange:time_stealing_potion/affected run scoreboard players add $affected_count orange.temp 1
...
execute on vehicle on origin run tag @s remove orange.time_stealing_potion.origin
